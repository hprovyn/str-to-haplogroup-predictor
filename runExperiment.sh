...
python3 "$createCSVinputForRF_py" "$treeFile" "$samplestsv.bgz" "$cladeFinderOutputFile" "$csvOutputForRFfile" "$panelHierarchyFile" "$createCSVinputMaxThreads" "$haplogroupClassConfigPath"
python3 "$rfExperiment_py" "$csvOutputForRFfile" "$modelOutputFile" "$panelHierarchyFile" "$predictionPolicyFileStem" "$modelPickleFileStem" "$utilityUnderSpecificityError" "$utilityOverSpecificityError" "$utilityCompletelyWrongError" "$utilityCorrect" "$experimentMapFileStem" "$percentMissingSTRThreshold" "$randomForestEstimators" "$randomForestMaxDepth"