...
python "$cladeFinder_py"  "$treeFile" "$samplestsv.bgz" "$cladeFinderOutputFile" "$cladeFinderMaxThreads"
python "$createCSVinputForRF_py" "$treeFile" "$samplestsv.bgz" "$cladeFinderOutputFile" "$csvOutputForRFfile" "$panelHierarchyFile" "$createCSVinputMaxThreads"
python "$rfExperiment_py" "$csvOutputForRFfile" "$modelOutputFile" "$panelHierarchyFile" "$predictionPolicyFileStem" "$modelPickleFileStem" "$utilityUnderSpecificityError" "$utilityOverSpecificityError" "$utilityCompletelyWrongError" "$utilityCorrect" "$experimentMapFileStem" "$percentMissingSTRThreshold"
